...
import type { CalTargets, FoodCategory, SavedFood } from "@/lib/cal/types";
...
export function ShiftlyCalLibraryView({
  savedFoods,
  targets,
}: {
  ...
  const [targetForm, setTargetForm] = useState<TargetFormState>({
    tdeeCalories: targets.tdeeCalories?.toString() ?? "",
    proteinTargetG: targets.proteinTargetG?.toString() ?? "",
    carbsTargetG: targets.carbsTargetG?.toString() ?? "",
    fatTargetG: targets.fatTargetG?.toString() ?? "",
    fiberTargetG: targets.fiberTargetG?.toString() ?? "",
  });
...
function TargetsPanel({
  ...
  return (
    <section className="rounded-lg border border-white/15 bg-black/20 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.14)] backdrop-blur-md">
