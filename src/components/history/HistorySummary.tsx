  ...
  return (
    <section className="mx-auto mb-5 max-w-7xl rounded-md border border-zinc-200 bg-white p-4 shadow-sm">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryTile label="Total weeks" value={String(stats.closedWeeks.length)} />
        <SummaryTile
          label="Total earnings"
          value={formatMoney(stats.totalEarningsCents)}
        />
        <SummaryTile label="Total spend" value={formatMoney(stats.totalSpendCents)} />
        <SummaryTile
    ...
    <div className="rounded-md border border-zinc-200 bg-zinc-50 p-4 lg:col-span-2">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-3">
        ...
        <AverageLine
          label="Cashflow"
          toneClass={
            cashflowCents === null ? "text-zinc-400" : cashflowWeeklyColor(cashflowCents)
          }
          value={cashflowCents}
        />
      </div>
  ...
  return (
    <div className="rounded-md border border-zinc-200 bg-zinc-50 p-4">
      ...
      {week ? (
        ...
        <>
          <p className="mt-2 text-lg font-semibold text-zinc-400">—</p>
          <p className="mt-1 text-sm text-zinc-500">No closed weeks yet</p>
        </>
      )}
    </div>
  );
