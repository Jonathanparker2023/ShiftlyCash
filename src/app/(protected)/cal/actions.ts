...
export async function generateMealOrderPromptAction(input?: {
  ...
  const lines = [
    ...
    "",
    "   **Nutritional research for the hidden gems (critical):** lesser-known restaurants rarely publish macros. You must source defensible macro estimates — do NOT force a chain-proxy if it doesn't fit the dish (e.g., Cava's flat-top chicken bowl ≠ Greek grilled skewers; they're different preps with different oil/marinade/portion profiles). Instead, use whichever of these sources actually applies to the dish:",
    "",
    "   - **USDA FoodData Central** (fdc.nal.usda.gov) — gold standard for whole-food and standard-preparation values (e.g., 'chicken kebab, grilled, USDA' for skewers; 'salmon fillet, baked, USDA' for a fish plate). Sum the ingredients.",
    "   - **The restaurant's own website / Instagram / printed menu** — some local spots DO publish basic info; check first.",
    "   - **MyFitnessPal user database** — for dishes commonly logged, MFP has community-validated entries. Use the top-rated entry for that exact dish name.",
    "   - **Cookbook / Serious Eats / food-blog recipes for the same dish** — if the dish is a known regional standard (gyro, pad see ew, masala dosa), find a published recipe with macros and sum from there.",
    "   - **Chain-equivalent ONLY when the prep is genuinely the same** — Wingstop grilled chicken is a fair proxy for another spot's grilled chicken; it is NOT a fair proxy for skewered marinated chicken. Don't reach.",
    "",
    "   Whichever source you use, **name it explicitly in the option's portioning section** so I can audit (e.g., 'Sourced via USDA FoodData Central: chicken kebab grilled + Greek salad ingredients summed; portion estimated at 7oz protein from dish photo').",
    "",
    "   Account for portion-size differences if the dish photo suggests a larger-than-chain serving. Account for known-heavy ingredients (butter, sugary sauces, fried prep) — adjust upward when present.",
    "",
    ...
    "",
    "6. **Gap-fill suggestions (required when gaps exist).** For your top-pick option, check what macros still leave a meaningful gap after eating it. For each gap, recommend a specific cheap home addition I can hit AFTER the meal to close it. Don't leave wide gaps without a fix. Use this rule set:",
    "",
    "   - **Protein gap >30g**: pick from egg whites (~3.6g per egg white), cottage cheese (~22g per cup), plain Greek yogurt (~17-22g per 6oz), whey protein shake (~25g per scoop), canned tuna in water (~22g per 3oz). Recommend a specific combination that closes the gap.",
    "   - **Protein gap 10-30g**: one of the above is enough — pick the lowest-sodium option (whey shake is best here for HBP).",
    "   - **Fiber gap >10g**: small green salad with vinegar (~3-5g), raspberries (~8g per cup), 1 apple with skin (~4g), 1/2 cup black beans (~7.5g), 1 tbsp chia seeds (~5g).",
    "   - **Calorie gap >400 AND it's late** (the user said this is their final meal): leave it — that's the cut deficit working. Don't recommend filling.",
    "   - **Calorie gap >400 AND it's early/mid-day**: recommend a protein-rich snack from the protein list above.",
    "   - **Sodium headroom unused**: no action — under-cap is good especially with HBP flag.",
    "   - **Added sugar headroom unused**: no action — unused sugar budget is a win.",
    "   - **Water gap**: just say 'drink ~X more oz of water before bed.'",
    "",
    "   Format the suggestions as a short section right under your top-pick reasoning, like:",
    "",
    "   ```",
    "   ## After this meal, to close the gaps:",
    "   - Protein: +25g still needed → 1 scoop whey + 1 egg white (closes to ~95% of daily target)",
    "   - Fiber: +18g still needed → 1 apple + small mixed greens salad (closes to ~80%)",
    "   - Water: +32 oz to hit your 80 oz daily goal before bed",
    "   ```",
    "",
    "   If NO meaningful gaps remain after the top pick, write 'No gaps to fill — this meal closes the day.'",
    "",
    "7. **At the very end, under a `## For logging` section, give a plain one-line macro summary for EACH of the 4 options in this exact format (no x/total, no rankings, just clean numbers I can paste into my food-logging AI):**",
    "",
    "   ```",
    ...
    "",
    "Browse now and return the 4 options with their DoorDash links, the ranking, the gap-fill suggestions, and the For logging block at the end.",
  ];
...
export async function generateHomeRecipePromptAction(): Promise<{
  ...
  const lines = [
    ...
    "",
    "8. **Also at the end, under a `## Shopping list` section, consolidate all the BUY items from all 4 recipes into ONE deduplicated shopping list grouped by store section** (Produce, Protein, Pantry, Dairy, Frozen, etc.). For each item, give exact amounts I'd need to make all 4 recipes (or whichever subset I pick — note which recipes each item is for in parentheses). This is the single thing I take to the store. Example:",
    "",
    "   ```",
    "   ## Shopping list",
    "",
    "   **Produce**",
    "   - 2 lemons (recipes 1, 3)",
    "   - 1 head broccoli (recipe 1)",
    "   - 1 bunch fresh thyme (recipes 1, 4)",
    "",
    "   **Protein**",
    "   - 2 lb boneless skinless chicken thighs (recipes 1, 2)",
    "   - 1 lb wild salmon fillet (recipe 4)",
    "",
    "   **Pantry / spices**",
    "   - Cumin (recipe 3) — likely have",
    "   - Low-sodium soy sauce (recipe 2)",
    "   ```",
    "",
    "   Tag items as 'likely have' if they're standard pantry. Only true must-buys should be unflagged.",
    "",
    "Browse now and return the 4 recipes with precise measurements, real source links, ranking, the For logging block, AND the consolidated Shopping list at the end.",
  ];
