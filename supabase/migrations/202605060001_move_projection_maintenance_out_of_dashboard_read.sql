-- Keep dashboard reads pure-ish: ensuring the active week may still create
-- structural rows when missing, but projection cleanup/application no longer
-- runs on every dashboard navigation. The app now triggers projection
-- maintenance after first paint via a server action.
