-- Ability autofill labels should not inherit arbitrary sticky labels from old
-- weeks. Generic 6h/12h Ability autofill stays blank; 10h Ability is Sunrise.
...
update public.earn_slots es
  set label = case
      when es.hours_or_units = 10 then 'Sunrise Cottage'
      else null
    end,
    updated_at = now()
from public.days d
...
where es.day_id = d.id
  ...
  and (
    (es.hours_or_units = 10 and coalesce(es.label, '') <> 'Sunrise Cottage')
    or (es.hours_or_units <> 10 and es.label is not null)
  );
...
begin
  ...
  with template_rows as (
    select
      ts.*,
      case
        when ts.job_type::text = 'ability' and ts.hours_or_units = 10
