-- Future-day spend projection now uses the all-time average of projection-
-- included closed weeks, not the most recent window.
...
declare
  ...
  v_average_week_spend numeric;
  v_per_day numeric;
  v_count integer;
begin
  if v_user_id is null then return 0; end if;

  select avg(spend_for_projection)
    into v_average_week_spend
  from public.v_projection_weeks
  where user_id = v_user_id
    and spend_for_projection is not null
    and spend_for_projection > 0;

  v_per_day := coalesce(round(v_average_week_spend / 7.0, 2), 0);
