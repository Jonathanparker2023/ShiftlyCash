alter type public.job_type add value if not exists 'ability_incentive' after 'ability';

alter table public.earn_slots
  add column if not exists incentive_mode text not null default 'none',
  add column if not exists incentive_rate numeric(10, 2) not null default 0,
  add column if not exists incentive_amount numeric(10, 2) not null default 0;
...
alter table public.earn_slots
  drop constraint if exists earn_slots_incentive_mode_check;
alter table public.earn_slots
  add constraint earn_slots_incentive_mode_check check (
    incentive_mode in ('none', 'rate', 'lump_sum')
    and incentive_rate >= 0
    and incentive_amount >= 0
  );

alter table public.template_slots
  drop constraint if exists template_slots_incentive_mode_check;
alter table public.template_slots
  add constraint template_slots_incentive_mode_check check (
    incentive_mode in ('none', 'rate', 'lump_sum')
    and incentive_rate >= 0
    and incentive_amount >= 0
  );
...
with earn_totals as (
  select
    ...
    coalesce(sum(
      case
        when es.job_type::text in ('ability', 'ability_incentive') and es.pay_type::text = 'regular'
          then es.hours_or_units * s.ability_regular_net_rate
        when es.job_type::text in ('ability', 'ability_incentive') and es.pay_type::text = 'overtime'
          then es.hours_or_units * s.ability_ot_net_rate
        when es.job_type::text in ('ability', 'ability_incentive') and es.pay_type::text = 'split'
      ...
      + case
          when es.job_type::text = 'ability_incentive' and es.incentive_mode = 'rate'
            then (es.hours_or_units * es.incentive_rate) * (1 - s.ability_withholding_rate)
          when es.job_type::text = 'ability_incentive' and es.incentive_mode = 'lump_sum'
    ...
    coalesce(sum(
      case
        when es.job_type::text in ('ability', 'ability_incentive', 'prestige', 'prestige_ilst')
          then case
  ...
  with updated as (
    update public.earn_slots es
      set
        ...
        incentive_mode = ts.incentive_mode,
        incentive_rate = ts.incentive_rate,
        incentive_amount = ts.incentive_amount,
        label = sl.label,
        source = 'template',
        updated_at = now()
    from public.days d
...
begin
  ...
  if exists (
    ...
    where parsed.day_index not between 0 and 6
      ...
      or (
        ...
        and (
          parsed.job_type not in ('ability', 'ability_incentive', 'prestige', 'prestige_ilst')
          or parsed.regular_hours <= 0
          or parsed.overtime_hours <= 0
        )
      )
  ...
  select distinct on (parsed.day_index, parsed.slot_index)
    ...
    case when parsed.job_type = 'ability_incentive' then parsed.incentive_mode else 'none' end,
    case when parsed.job_type = 'ability_incentive' and parsed.incentive_mode = 'rate' then parsed.incentive_rate else 0 end,
    case when parsed.job_type = 'ability_incentive' and parsed.incentive_mode = 'lump_sum' then parsed.incentive_amount else 0 end
  from (
    select
      (payload.slot ->> 'dayIndex')::integer as day_index,
      (payload.slot ->> 'slotIndex')::integer as slot_index,
